    ...
    def validate(self, data):
        ...
        if password != password_check:
            raise serializers.ValidationError(
                {"error": "password가 일치하지 않습니다. 확인해주세요."})
        if len(password) < 8 or len(password) > 32:
            raise serializers.ValidationError(
                {"error": "password는 8-32자리여야 합니다."})
        if not re.match(r'^(?=.*[A-Za-z])(?=.*\d)(?=.*[!#$%^&?])[\w!#$%^&?]{8,32}$', password):
            raise serializers.ValidationError(
                {"error": "password는 영문, 숫자, 특수문자(!#$%^&?) 조합 8-32 자리여야 합니다."})
    ...
    def validate(self, data):
        ...
        if nickname:
            if User.objects.filter(nickname=nickname).exists():
                raise serializers.ValidationError(
                    {"error": "이미 존재하는 nickname 입니다."})
            if not re.match(r'^[a-zA-Z0-9]{1,10}$', nickname):
                raise serializers.ValidationError(
                    {"error": "닉네임을 10자 이하의 영문자와 숫자 조합으로 설정해 주세요."})
...
class SigninSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        try:
            ...
            raise serializers.ValidationError(
                {"errors": "가입한 회원이 아닙니다. email 확인 또는 회원가입을 진행해주세요."})

        if not user.check_password(password):
            raise serializers.ValidationError(
                {"errors": "email, password 확인해주세요."})
